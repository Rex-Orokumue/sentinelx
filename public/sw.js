...
//
// This worker is now registered unconditionally on every page load
// (registerServiceWorker() in useFCM.ts, required for PWA installability),
// not only after a player opts into push — so it must survive being
// registered with NO query params at all. Firebase init is guarded behind
// an actual apiKey being present; without one, this worker still installs
// and activates normally (offline caching keeps working), it just skips
// push messaging setup instead of crashing on an invalid config.
const params = new URLSearchParams(self.location.search)
const firebaseApiKey = params.get('apiKey')
