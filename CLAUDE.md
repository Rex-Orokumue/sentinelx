...
- After verification, `resolveCallbackRedirect({ type, next })` decides the destination: `type=recovery` → `/reset-password`; `type=email_change` → `/dashboard/settings?email=changed`; otherwise the `next` param (default `/dashboard`).
- The Supabase **email templates** (Confirm signup, Reset password, Change Email Address) must point at `{{ .SiteURL }}/auth/confirm` with `token_hash={{ .TokenHash }}&type={{ .Type }}&next=…`. This is dashboard configuration, not code. The `{{ .SiteURL }}` prefix is required — a relative `href` cannot be resolved by an email client, and the stock `{{ .ConfirmationURL }}` is the fragment-based flow that breaks here.
- **Changing an email address** (`changeEmail()` in `lib/auth/actions.ts`) requires the current password, re-checked by `verifyPassword()` in `lib/auth/reauth.ts` through a `persistSession: false` client so the live session's cookies aren't rotated mid-request. Supabase's **"Secure email change" is deliberately OFF**: one link goes to the *new* address only, because the usual reason to change an address is that the old one is unreachable — the password is what replaces the old inbox as proof of ownership. Google-only accounts (no `email` identity) are told to set a password first via the existing reset flow, whose link lands in the *current* inbox. New addresses are checked against `banned_identifiers`, the same blocklist signup enforces. Nothing changes until the link is opened; until then Supabase holds the address in `user.new_email`.
...
9. **Sensitive tables are written only by the service role.** `profiles`, `tournament_registrations`, `withdrawal_requests`, `match_results`, `friendly_matches` and `friendly_match_results` have no INSERT/UPDATE/DELETE grant for `anon`/`authenticated` — use `createAdminClient()` after verifying the caller with `getUser()`. `lib/security/write-paths.test.ts` enforces it.
10. **Profile columns are private by default.** `anon`/`authenticated` can SELECT only the allow-listed columns in `20260918200000_lock_down_profiles_and_write_paths.sql`. Private: `phone, whatsapp_number, notification_prefs, referred_by, deletion_requested_at` — read them with the service role for the caller's own row, or for staff/counterpart flows after an explicit authorisation check. Adding a public column needs `GRANT SELECT (col)`. Never `select('*')` on `profiles`.
11. **Mobile API endpoints live under `/api/mobile/v1` and are defined with `defineEndpoint()`** (`lib/mobile-api/define-endpoint.ts`) — bearer auth, `{data}`/`{error}` envelope, min-app-version gate, and OpenAPI metadata come for free. Never write a route handler under `app/api/mobile/v1/**` by hand. Conventions and how to add an endpoint: `docs/superpowers/specs/2026-09-18-mobile-api-v1-conventions.md`. After adding/changing an endpoint, run `npm run openapi` and commit `openapi/mobile-v1.json` — the Flutter mobile repo (`sentinelx_mobile`) builds against that contract.
12. **Money/score/state-creating mobile POSTs require `idempotent: true`** on their `defineEndpoint()` call, backed by `api_idempotency_keys` and `runIdempotent()` (`lib/mobile-api/idempotency.ts`) — Stripe-style claim/fill/reclaim, same key always replays the same stored response (success or error). Full design and the generation-gated race fix: `docs/superpowers/specs/2026-09-22-mobile-phase2a-tournaments-registration-design.md` §4. Don't reimplement this per-endpoint — every future money-creating write (deposit, withdraw, escrow, wager, stake, submit result) reuses the same primitive.
