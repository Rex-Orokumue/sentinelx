...
describe('createComment notifications — match results', () => {
  it('notifies both players in the match', async () => {
    ...
    expect(pushToPlayer).toHaveBeenCalledWith('player-a', 'post_comment', expect.anything(), expect.anything())
    expect(pushToPlayer).toHaveBeenCalledWith('player-b', 'post_comment', expect.anything(), expect.anything())
  })
...
describe('createComment notifications — announcements', () => {
  it('notifies staff, since an announcement has no author', async () => {
    ...
    expect(pushToPlayer).toHaveBeenCalledWith('admin-1', 'post_comment', expect.anything(), expect.anything())
    expect(pushToPlayer).toHaveBeenCalledWith('mod-1', 'post_comment', expect.anything(), expect.anything())
  })
