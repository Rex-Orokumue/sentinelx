...
import { matchWinnerId, sideAId, sideBId, type AdvanceMatch } from './advancement'
...
//
// playerToEntityId (optional): for a team-vs-team tournament, a match's
// sides are squad ids, not player ids — this map translates an individual
// player to their squad before the lookup, so the OUTPUT stays keyed by the
// original activePlayerIds either way. Every roster member of the champion
// squad ends up with band 'champion', the same way a solo champion does.
export function bandsForPlacements(
  ...
  for (const round of ROUND_ORDER) {
    ...
    for (const match of roundMatches) {
      ...
      const a = sideAId(match)
      const b = sideBId(match)
      ...
      if (round === 'final') {
        if (match.status === 'forfeited') {
          ...
          if (a) band.set(a, 'runner_up')
          if (b) band.set(b, 'runner_up')
          continue
        }
      ...
      if (match.status === 'forfeited') {
        if (a) band.set(a, round as PlacementBand)
        if (b) band.set(b, round as PlacementBand)
        continue
      }
  ...
  return activePlayerIds.map((playerId) => {
    const entityId = playerToEntityId?.get(playerId) ?? playerId
    return { playerId, band: band.get(entityId) ?? 'non_advancer' }
  })
}
  ...
  for (let i = 0; i < ROUND_ORDER.length; i++) {
    ...
    for (const match of roundMatches) {
      ...
      if (match.status === 'bye') {
        ...
        // finishing).
        const soloSide = a ?? b
        if (soloSide && nextRound) floor.set(soloSide, nextRound)
        continue
      }
      if (a) floor.set(a, round as PlacementBand)
      if (b) floor.set(b, round as PlacementBand)
    }
  }

  return activePlayerIds.map((playerId) => {
    const entityId = playerToEntityId?.get(playerId) ?? playerId
    return { playerId, band: floor.get(entityId) ?? 'non_advancer' }
  })
}
