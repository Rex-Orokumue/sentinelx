...
2. Additionally fetch the season's still-`active`-status tournaments of either season-eligible type (`community_club` or `masters` — both, not community_club-only: real locked-in `season_ranking_points` rows are already written for both types per the original Phase 2 plan's Global Constraints #3, so a player's Champions-Cup-qualifying total can legitimately include points from a masters tournament in the same season; restricting the new provisional layer to community_club-only would make it inconsistent with the real numbers it sits alongside) and their matches. `getSeasonLeaderboard` itself is confirmed to be exclusively the Champions Cup path — `lib/seasons/invitation-actions.ts`'s `leaderboardFor` calls `getMonthlyLeaderboard` when `tournament_type === 'masters'` and `getSeasonLeaderboard` otherwise — this is about which SOURCE tournaments feed the Champions Cup total, not about which qualification path is being computed.
3. For each such tournament, run `guaranteedBandsForPlacements` against its current match state, and for every registered player who does **not already have a real `season_ranking_points` row for that tournament**, compute `pointsForBand(tournament.tournament_type, guaranteedBand)` as their provisional total.
...
- **Tournament types other than community_club/masters:** already excluded upstream by the same gate `getSeasonLeaderboard` uses today (`.in('tournament_type', ['community_club', 'masters'])`) — provisional computation never runs for champions_cup/open tournaments themselves (a champions_cup tournament's own players aren't season-leaderboard participants; champions_cup is what the season leaderboard *qualifies you for*, not itself part of it).
